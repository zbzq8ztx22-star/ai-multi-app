...
export default function AccountingOperations({ businessId, accounts, onPosted }) {
  ...
  return <div className="space-y-6">
    {error && <div className="p-3 bg-red-900/20 border border-red-800 rounded text-red-200">{error}</div>}
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
      <form className="card space-y-3" onSubmit={submit(async () => { await apiPost('/api/accounting/contacts', { ...contact, business_id: businessId }); setContact({ name: '', contact_type: 'customer', email: '' }) })}>
        <h2 className="font-semibold">Customers & vendors</h2><div className="grid grid-cols-3 gap-2"><input className="input-field" required placeholder="Name" value={contact.name} onChange={e => setContact({ ...contact, name: e.target.value })} /><select className="input-field" value={contact.contact_type} onChange={e => setContact({ ...contact, contact_type: e.target.value })}><option value="customer">Customer</option><option value="vendor">Vendor</option><option value="both">Both</option></select><input className="input-field" type="email" placeholder="Email" value={contact.email} onChange={e => setContact({ ...contact, email: e.target.value })} /></div><button className="btn-primary flex gap-2"><Plus size={16} /> Add contact</button>
        <div className="space-y-1">{contacts.map(c => <div key={c.id} className="flex justify-between text-sm p-2 bg-gray-800 rounded"><span>{c.name}</span><span className="text-gray-400 capitalize">{c.contact_type}</span></div>)}</div>
      </form>
