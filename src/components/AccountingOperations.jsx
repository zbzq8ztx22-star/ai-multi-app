...
export default function AccountingOperations({ businessId, accounts, onPosted }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="card"><h2 className="font-semibold flex gap-2 mb-3"><Receipt size={18} /> Recent operations</h2>{invoices.map(i => <div key={`i${i.id}`} className="flex justify-between py-2 border-b border-gray-800"><span>Invoice {i.invoice_number} · {i.customer_name}</span><span>{money(i.amount)} · {i.status}</span></div>)}{expenses.map(e => <div key={`e${e.id}`} className="flex justify-between py-2 border-b border-gray-800"><span>Expense · {e.description}</span><span>{money(e.amount)}</span></div>)}</div>
  </div>
}
